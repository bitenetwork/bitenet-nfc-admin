...
import {
  PagePointsTransationInputs,
  PagePointsTransationOutputs,
...
export default function RechargeDetail({ brandId, open, onClose }: Prop) {
  ...
  const [queryOption, setQueryOption] = useState<PagePointsTransationInputs>({
    brandId,
    page: 1,
    pageSize: 10,
  });
  ...
  const { data, isLoading, refetch } =
    api.restaurantWalletPoints.pageTransation.useQuery(queryOption);

  const { data: balance, refetch: refetchBalance } =
    api.restaurantWalletPoints.getBalance.useQuery({ brandId });
