...
import { AddRechargeInputs } from "~/trpc/admin/account/types";
...
export default function AddRecharge({ brandId, open, onClose }: Prop) {
  ...
  const { mutateAsync: addRecharge, isLoading } =
    api.restaurantWalletPreRecharge.addRecharge.useMutation();

  const onFinish = (inputs: AddRechargeInputs) => {
    console.log(inputs);
    addRecharge(inputs)
      .then(onClose)
