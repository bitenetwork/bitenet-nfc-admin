...
export default function EditBrand({ id, open, onClose }: Prop) {
  const { Option } = Select;
  ...
  const { mutateAsync: updateBrand, isLoading } =
    api.brand.updateBrand.useMutation();

  const onFinish = (data: CreateBrandInputs) => {
    if (id) {
      if (data.levelType !== "EXPIRED" && !data.expiredDate) {
  ...
  useEffect(() => {
    if (id) {
      utils.brand.findBrandById.fetch({ id }).then((data) => {
        form.setFieldsValue({
          ...data,
          expiredDate: data
            ? data.expiredDate
              ? moment(data.expiredDate).endOf("day")
              : null
            : null,
        });
