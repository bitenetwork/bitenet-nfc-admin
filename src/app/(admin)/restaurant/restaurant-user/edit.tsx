...
export default function EditRestaurantUser({ id, open, onClose }: Prop) {
  const { Option } = Select;
  ...
  const { mutateAsync: updateRestaurantUser, isLoading } =
    api.restaurantUser.updateRestaurantUser.useMutation();

  const onFinish = (data: CreateRestaurantUserInputs) => {
    if (id) {
      updateRestaurantUser({ id, data })
        .then(onClose)
  ...
  useEffect(() => {
    if (id) {
      utils.restaurantUser.findRestaurantUserById.fetch({ id }).then((data) => {
        form.setFieldsValue(data);
        setImageUrl(data?.avatar || "");
        form.setFieldsValue({ confirmPassword: data?.password || "" });
      });
    }
  }, [id, open]);
        ...
        >
          ...
          <Row gutter={16}>
            ...
            <Col span={12}>
              ...
              >
                <Input disabled={true} placeholder="Please enter account" />
              </Form.Item>
            </Col>
          </Row>
          <Row gutter={16}>
